...
##################################### Load gene expression file as covariate
covar=read.table("etraits_23610.txt",sep="\t",header=T)
candidate.one="GRMZM2G######"
covar.one=covar[,which(names(covar)==candidate.one)]
...
out.mqm= fitqtl(geno, pheno.col=pheno.col, qtl=qtl, formula= formula1, method= "hk", dropone=T, get.ests=T, covar=covar)
summary(out.mqm)
